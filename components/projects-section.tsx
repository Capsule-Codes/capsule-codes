...
function BentoCard({ project, featured = false, language, t }: BentoCardProps) {
  ...
  return (
    <Link href={`/projects/${project.id}`} className="block h-full group">
      <article
        className={`relative overflow-hidden rounded-2xl bg-center border border-[color:var(--ink-line)] transition-[transform,border-color] duration-300 group-hover:-translate-y-0.5 group-hover:border-[color:var(--brand-cyan)]/40 ${
          featured ? "h-full min-h-[400px]" : "aspect-[4/3]"
        } ${thumb.portrait ? "" : "bg-cover"}`}
        style={{
          backgroundImage:
            !thumb.portrait && image ? `url(${image})` : preset.background,
        }}
      >
